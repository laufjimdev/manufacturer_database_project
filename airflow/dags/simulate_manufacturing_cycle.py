...
def simulate_manufacturing_cycle():
    ...
    @task
    def run_returns_simulation(**context):
        params = context["params"]
        returns_count = params["returns_count"]
        connection = get_connection()
        try: 
            simulate_returns(connection, returns_count)
            connection.commit()
        except Exception as e:
            connection.rollback()
            raise Exception(f"Failed to generate return orders, error: {e}") 
        finally:
            connection.close()
